...
class TestWithdrawCase(unittest.TestCase):
    ...
    excle=ReadExcle(os.path.join(DATA_DIR,"apicases.xlsx"),"withdraw")
    ...
    def test_withdraw(self,case):
        # 1.请求行 url，method
        withdraw_url=conf.get("env","base_url")+case["url"]
        withdraw_method=case["method"]

        # 2.请求头 headers
        withdraw_headers=eval(conf.get("env","headers_v2"))
        if case["interface"]!="login":
            withdraw_headers["Authorization"]=getattr(TestData,"token_value")

        # 3.请求体 data
        withdraw_data=eval(replace_data(case["data"]))

        # 4.预期结果和回写数据的行数
        withdraw_expected=eval(case["expected"])
        row_num=case["case_id"]+1

        # 5.取现前先检查数据库当中的余额start_money
        if case["check_sql"]:
        ...
        # 6.发起请求，获取实际返回结果
        response=self.http.send_requests_v2(url=withdraw_url,method=withdraw_method,headers=withdraw_headers,json=withdraw_data)
        json_data=response.json()

        # 7.取现后再检查数据库当中的余额end_money
        if case["check_sql"]:
            sql="SELECT leave_amount FROM futureloan.member WHERE mobile_phone={}".format(conf.get("test_data", "username"))
            end_money=self.mysql.get_one(sql)["leave_amount"]

        # 8.根据接口来判断是否需要提取member_id,token值
        if case["interface"]=="login":
            # member_id
        ...
        # 9.断言
        try:
            self.assertEqual(withdraw_expected["code"],json_data["code"])
            self.assertEqual(withdraw_expected["msg"],json_data["msg"])
            if case["check_sql"]:
                self.assertEqual(start_money-end_money,Decimal(str(withdraw_data["amount"])))
